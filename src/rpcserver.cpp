...
int64_t AmountFromValueNonRounded(const Value& value)
{
    double dAmount = value.get_real();
    if (dAmount <= 0.0 || dAmount > MAX_MONEY)
        throw JSONRPCError(RPC_TYPE_ERROR, "Invalid amount");
    int64_t nAmount = (dAmount * COIN);
    if (!MoneyRange(nAmount))
        throw JSONRPCError(RPC_TYPE_ERROR, "Invalid amount");
    return nAmount;
}
...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "Stop BuzzCoin server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "BuzzCoin server stopping";
}

Value stopnode(const Array& params, bool fHelp)
{
    // Accept the deprecated and ignored 'detach' boolean argument
    if (fHelp || params.size() > 1)
    throw runtime_error(
...
{ //  name                      actor (function)         okSafeMode threadSafe reqWallet
  //  ------------------------  -----------------------  ---------- ---------- ---------
    ...
    { "stopnode",               &stopnode,                   true,      true,      false },
    ...
    { "getblocksbynumber",      &getblocksbynumber,      false,     false,     false },
    { "getblockhashbynumbers",  &getblockhashbynumbers,  false,     false,     false },
    { "getblockhash",           &getblockhash,           false,     false,     false },
    { "getblockstats",          &getblockstats,          false,     false,     false },
...
void StartRPCThreads()
{
         ...
         (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"])) && Params().RequireRPCPassword())
    {
        ...
        string strWhatAmI = "To use buzzcoind";
