...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "BuzzCoin " + FormatFullVersion();
        ...
        if (HaveNameProxy()) {
            ...
            if (LookupHost(seed.host.c_str(), vIPs))
            {
                BOOST_FOREACH(CNetAddr& ip, vIPs)
                {
                    ...
                    found++;
                    if (fDebug) {
                        LogPrintf("dnsseed added address %s, found from DNS seeds\n", addr.ToString());
                   }
                }
            }
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. BuzzCoin is probably already running."), addrBind.ToString());
        else
