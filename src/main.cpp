...
const string strMessageMagic = "BuzzCoin Signed Message:\n";
...
int64_t GetMinFee(const CTransaction& tx, unsigned int nBlockSize, enum GetMinFee_mode mode, unsigned int nBytes)
{
	...
	int64_t TransactionFeeDivider = 1;
//  time_t t=time(NULL); // TA diabled uses time of tx now
    time_t t=tx.nTime;
    if( t > ForkTiming) {
    ...
    if (fDebug) {
        LogPrintf("GetMinFee() : Checking Transaction\n%s", tx.ToString());
    }
    ...
    BOOST_FOREACH(const CTxOut& txout, tx.vout) {
        ...
        if (txout.nValue < 100) {
            nMinFee += nBaseFee;
            if (fDebug)
                LogPrintf("GetMinFee() : txout < 100 adding nBaseFee=%d\n", nBaseFee);
        }
    }
    ...
    if(t > PercentageFeeRelayBegin || (t > PercentageFeeSendingBegin && mode==GMF_SEND) )  
    {
        ...
        if (fDebug)
            LogPrintf("GetMinFee() : t %d > PercentageFeeRelayBegin %d \n", t, PercentageFeeRelayBegin);
        ...
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            ...
            if (t > Fork3) { 
                nNewMinFee = (txout.nValue / 100000) * 1;
               if (fDebug)
                    LogPrintf("GetMinFee() : t %d > Fork3 %d nMinFee=%d nNewMinFee=%d \n", t, Fork3, nMinFee, nNewMinFee);
            } else {
                nNewMinFee = (txout.nValue / 100) * 25;
                if (fDebug)
                    LogPrintf("GetMinFee() : t %d <= Fork3 %d, nMinFee=%d nNewMinFee=%d \n", t, PercentageFeeRelayBegin, nMinFee, nNewMinFee);
            }
        }
        nMinFee += nNewMinFee;
    }
    ...
    // TA fix for stakes, they do not need a fee after the ThreeTwoFix
    if (pindexBest->nHeight >= Params().ThreeTwoFix() && tx.IsCoinStake()) {
        nMinFee = 0;
        if (fDebug)
            LogPrintf("GetMinFee() : height=%d > ThreeTwoFix(%d)\n", pindexBest->nHeight, Params().ThreeTwoFix());
    }

    if (fDebug) {
        LogPrintf("GetMinFee() : txoutValue=%s txoutValue=%d fee=%s fee=%d\n",
                  FormatMoney(txoutCombinedValue), txoutCombinedValue, 
                  FormatMoney(nMinFee), nMinFee);
    }
...
bool AcceptToMemoryPool(CTxMemPool& pool, CTransaction &tx, bool fLimitFree, bool* pfMissingInputs)
{
...

    {
        ...
        if ((fLimitFree && nFees < txMinFee) || (!fLimitFree && nFees < MIN_TX_FEE))
            ...
            LogPrint("mempool", "AcceptToMemoryPool : accepted %s (poolsz %u) size %u txfee %d \n",
                     hash.ToString(),
                     pool.mapTx.size(),
                     nSize,
                     nFees);
...
int64_t GetProofOfWorkReward(int64_t nFees, CBlockIndex* pindex)
{
    ...
    time_t t = pindex->nTime > 0 ? pindex->nTime : (pindex->pprev ? pindex->pprev->nTime : time(NULL)) ;
    int64_t nMoneySupplyPIndex = pindex->nMoneySupply;
    int64_t nHeightPIndex = pindex->nHeight;
    int64_t nMintPIndex = pindex->nMint;
    bool nUsingPPprev = pindex->nMoneySupply == 0 ? true : false;

    int64_t nMoneySupply = pindex->nMoneySupply == 0 ? pindex->pprev->nMoneySupply : pindex->nMoneySupply;
    int64_t nHeight = pindex->nMoneySupply == 0 ? pindex->pprev->nHeight : pindex->nHeight;
    int64_t nMint = pindex->nMoneySupply == 0 ? pindex->pprev->nMint : pindex->nMint;
    int64_t nCurrentHeight = pindex->nMoneySupply == 0 ? pindex->pprev->nHeight : pindex->nHeight;

    // double fCurrentSupply = GetCoinSupplyFromAmount(nMoneySupply);
    double fCurrentSupply = static_cast<double>(nMoneySupply) / static_cast<double>(COIN);

    if (fDebug) {
        LogPrint("creation", "GetProofOfWorkReward(): nHeightPIndex=%d nMintPIndex=%.8f nMoneySupplyPIndexFormatted=%s nMoneySupplyPIndex=%.8f\n", nHeightPIndex, nMintPIndex, FormatMoney(nMoneySupplyPIndex), nMoneySupplyPIndex);
        if (nUsingPPprev) 
            LogPrint("creation", "GetProofOfWorkReward(): nHeight=%d nMint=%.8f fCurrentSupply=%s nMoneySupply=%.8f\n", nHeight, nMint, FormatMoney(nMoneySupply), nMoneySupply);
    }
    ...
    if (TestNet()) {
        ...
        if(pindexBest->nHeight <= 300) {
            if (fDebug) {
                LogPrint("creation","GetProofOfWorkReward(): ONE_BILLION premine at block %d.\n", pindexBest->nHeight);
            }
...
/*     // if we are equal to or over 20b, we return no subsidy.
    if (nMoneySupply > MAX_MONEY) {
        if (fDebug) {
            LogPrint("creation", "GetProofOfWorkReward(): currentSupply=%.8f nMoneySupply=%.8f, returning 0 mining subsidy. fees are %.8f\n", fCurrentSupply, nMoneySupply, nFees);
        }
        return 0 * COIN;
    }
 */
    ...
    if (t > SOME_RANDOM_LEGACY_FORK) {
        ...
        // 20 bil, so chop it.
        double supplyAfterReward = fCurrentSupply + MINING_REWARD;
        int64_t nMoneySupplyAfterReward = nMoneySupply + nSubsidy;
        int64_t nMaxReward = MAX_MONEY > nMoneySupplyAfterReward ? nSubsidy : ( nMoneySupply < MAX_MONEY ) ? MAX_MONEY - nMoneySupply : 0;
        LogPrint("creation", "GetProofOfWorkReward(): supplyAfterReward %.8f  nMoneySupplyAfterReward=%d  nMaxReward=%d\n", 
                    supplyAfterReward, nMoneySupplyAfterReward, nMaxReward);

        if ( nMaxReward < nSubsidy) {
            if (fDebug) {
                LogPrint("creation", "GetProofOfWorkReward(): nSubsidy %.8f exceeds max supply, setting to %.8f %d.\n",
                    supplyAfterReward, ((TWENTY_BILLION - fCurrentSupply)), (TWENTY_BILLION - fCurrentSupply) * COIN);
                LogPrint("creation", "GetProofOfWorkReward(): nSubsidy %.8f exceeds max supply, setting to %.8f.\n",
                    nMoneySupplyAfterReward, nMaxReward);

            }
            // WORKAROUND for calculation error for max supply, overshoots the supply by few hundret BUZZ cent
            if ( nCurrentHeight >= Params().MaxSupplyFix() && nCurrentHeight <= Params().ThreeTwoFix() ) {
                if (fDebug) {
                    LogPrint("creation", "GetProofOfWorkReward(): MaxSupplyFix() ThreeTwoFix() nSubsidy %.8f exceeds max supply, setting to %.8f.\n",
                        supplyAfterReward, nMaxReward);
                }
                nMaxReward += 1000; 
            }
        ...
        if (nMoneySupply <= MAX_MONEY) {
            LogPrint("creation", "GetProofOfWorkReward() : height=%d create=%s nSubsidy=%d fee=%d time=%s\n",
                     nCurrentHeight, FormatMoney(nSubsidy), nSubsidy, nFees, DateTimeStrFormat("%x %H:%M:%S", pindexBest->GetBlockTime()));
            LogPrint("creation", "GetProofOfWorkReward() : height=%d SOME_RANDOM_LEGACY_FORK=%d pindexBestTime=%d\n",nCurrentHeight,SOME_RANDOM_LEGACY_FORK,pindexBest->GetBlockTime());
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees, CBlockIndex* pindex)
{
    ...
    if (fDebug) {
        LogPrint("creation", "GetProofOfStakeReward(): nHeight=%d nMint=%d fCurrentSupply=%.8f fCurrentSupply2=%s nMoneySupply=%d\n", nHeight, nMint, fCurrentSupply, FormatMoney(fCurrentSupply), nMoneySupply);
    }

    if (nMoneySupply >= MAX_MONEY) {
        LogPrint("creation", "GetProofOfStakeReward(): Max supply reached or exceeded, no more staking rewards at fCurrentSupply=%.8f fCurrentSupply2=%s nMoneySupply=%d\n", fCurrentSupply, FormatMoney(fCurrentSupply), nMoneySupply);
        return 0;
    }
    ...
    // it is nice to give this reward still, just ensure that it does not exceed
    // 20 bil, so chop it.
    if (nSubsidy + nMoneySupply > MAX_MONEY) {
        if (fDebug) {
            LogPrint("creation", "GetProofOfWorkReward(): nSubsidy exceeds max supply, setting to %.8f.\n",
                MAX_MONEY - nMoneySupply);
        }

        nSubsidy = MAX_MONEY - nMoneySupply;
    }
//    if (fDebug)
        LogPrint("creation", "GetProofOfStakeReward(): height=%d nSubsidy=%s nSubsidy2=%s nCoinAge=%d nFees=%d\n",
                  pindex->nHeight, FormatMoney(nSubsidy), nSubsidy, nCoinAge, nFees);
    ...
    if (!IsCoinBase())
    {
        ...
        if (!IsCoinStake())
        {
            ...
            if (nTxFee < nRequiredFee && (pindexBest->nTime) > 1454926540) {
                ...
                // Exceptions to fix bogus fees in existing blocks
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfWork())
    {
        ...
        if (fDebug)
            LogPrintf("ConnectBlock() : coinbase reward (actual=%d vs calculated=%d)  nFees=%d  nHeight=%d\n",
                      vtx[0].GetValueOut(),
                      nReward, nFees, pindex->nHeight);
    ...
    if (IsProofOfStake())
    {
        ...
        if (nStakeReward > nCalculatedStakeReward)
            return DoS(100, error("ConnectBlock() : coinstake pays too much %d (actual=%d vs calculated=%d)",
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version")
    {
        ...
        if (
            ...
            || ( pfrom->nVersion < MIN_PEER_PROTO_VERSION_V3_2 )
            )
        {
            // disconnect from peers older than this proto version
            LogPrintf("ProcessMessage(): partner %s using obsolete version %i; disconnecting\n", pfrom->addr.ToString(), pfrom->nVersion);
            pfrom->fDisconnect = true;
            return false;
        } else if (fDebug) {
            LogPrintf("ProcessMessage(): partner %s using version %i\n", pfrom->addr.ToString(), pfrom->nVersion);
        }
