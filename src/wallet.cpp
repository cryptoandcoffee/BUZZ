    ...
    // restored from backup or the user making copies of wallet.dat.
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            ...
            if (mi != mapWallet.end())
            {
                ...
                {
                    LogPrintf("WalletUpdateSpent found spent coin %s %d %s\n", FormatMoney(wtx.GetCredit()), wtx.GetCredit(), wtx.GetHash().ToString());
    ...
    while (fRepeat)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (txdb.ReadTxIndex(wtx.GetHash(), txindex))
            {
                ...
                if (fUpdated)
                {
                    LogPrintf("ReacceptWalletTransactions found spent coin %s %d BUZZ %s\n", FormatMoney(wtx.GetCredit()), wtx.GetCredit(), wtx.GetHash().ToString());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                }
            }
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
        ...
        BOOST_FOREACH(const CTxIn& txin, txNew.vin)
                {
                    ...
                    if(txdb.ReadDiskTx(prevout.hash, prev))
                    {
                        if (prevout.n < prev.vout.size())
                        {
                            ...
                            {
                                address_str = CBitcoinAddress(address).ToString();
                            }
                            
                            LogPrintf("CreateCoinStake() : Inputs: address %s nCredit=%d \n",
                                        address_str.c_str(), vout.nValue );
                                                                }
                    }
                }
    }
    ...
    // Calculate coin age reward
    {
        ...
        // TA: commented out, solves issue that no POS available after we reached max supply
        // if (nReward <= 0)
        //    return false;
...
//            if (nNet > MIN_TX_FEE ) {
                if (txNew.vout.size() == 4) { // split stake
                    ...
                    if (fDebug)
                        LogPrintf("CreateCoinStake() : DevFee: nCredit=%d  nReward=%d  tx1=%d  tx2=%d  nDevFee=%d\n",
                                  nCredit, nReward, txNew.vout[1].nValue, txNew.vout[2].nValue,
                                  txNew.vout[3].nValue);
                } else { // non-split stake
...
//            }
            ...
            if (txNew.vout.size() == 3) { // split stake
                ...
                if (fDebug)
                    LogPrintf("CreateCoinStake() : NoDevFee: nCredit=%d  nReward=%d  tx1=%d\n",
                              nCredit, nReward, txNew.vout[1].nValue);
            }
        }
        ...
        {
            if (fDebug)
                LogPrintf("CreateCoinStake() : fee for coinstake %s %d\n",
...
void CWallet::FixSpentCoins(int& nMismatchFound, int64_t& nBalanceInQuestion, bool fCheckOnly)
{
    ...
    BOOST_FOREACH(CWalletTx* pcoin, vCoins)
    {
        ...
        for (unsigned int n=0; n < pcoin->vout.size(); n++)
        {
            ...
            {
                LogPrintf("FixSpentCoins found lost coin %s BUZZ %s[%d], %s\n",
            ...
            {
                LogPrintf("FixSpentCoins found spent coin %s BUZZ %s[%d], %s\n",
                    FormatMoney(pcoin->vout[n].nValue), pcoin->GetHash().ToString(), n, fCheckOnly? "repair not attempted" : "repairing");
                nMismatchFound++;
                nBalanceInQuestion += pcoin->vout[n].nValue;
                if (!fCheckOnly)
