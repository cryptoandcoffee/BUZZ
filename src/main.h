...
inline unsigned int GetTargetSpacing(int nHeight) { return IsProtocolV2(nHeight) ? 125 : 120; }
...
inline int64_t GetCoinYearReward(CBlockIndex* pindex) {
    ...
    if (fDebug)
        LogPrintf("GetCoinYearReward(): currentSupply=%.8f currentSupply2=%s nMint=%d nMoneySupply=%.8f currentHeight=%d\n", fCurrentSupply, FormatMoney(nMoneySupply), nMint, nMoneySupply, nCurrentHeight);

    // no reward after 20b
    if (fCurrentSupply > TWENTY_BILLION) {
        if (fDebug)
            LogPrintf("GetCoinYearReward(): Staking reward disabled. currentSupply=%.8f currentSupply2=%s nMoneySupply=%.8f\n", fCurrentSupply, FormatMoney(nMoneySupply), nMoneySupply);
    ...
    if (nCurrentHeight < Params().ThreeOhFix()) {
        if (fDebug)
            LogPrintf("GetCoinYearReward(): yearReward=%.8f\n broken after stability fork, before threeohfix\n", max(1200 - (1200 * (fCurrentSupply/MAX_MONEY)), 2.5));
        ...
        if (fDebug)
            LogPrintf("GetCoinYearReward(): yearReward=%.8f threeohfix\n", (1200 - (1200 * (fCurrentSupply/TWENTY_BILLION))) * CENT);
        
        return (1200 - (1200 * (fCurrentSupply/TWENTY_BILLION))) * CENT;
    }
...
inline int GetMinStakeAge(CBlockIndex* pindex)
{
    ...
    int64_t nMoneySupply = pindex->pprev ? pindex->pprev->nMoneySupply : pindex->nMoneySupply;
    int64_t nMint = pindex->pprev ? pindex->pprev->nMint : pindex->nMint;
    double fCurrentSupply = GetCoinSupplyFromAmount(nMoneySupply + nMint);

    // if not yet reaching activation block and we are NOT on test net
    // this will return the original maturation time of 8 hours.
    if (nCurrentHeight <= Params().StabilitySoftFork()) {
        if (fDebug)
            LogPrintf("GetMinStakeAge(): fCurrentSupply=%.8f nCurrentHeight=%d nMint=%d minStakeAge=%d\n", fCurrentSupply, nCurrentHeight, nMint, nHours * 60 * 60);
    ...
    if (nCurrentHeight < Params().ThreeOhFix()) {
        ...
        // satoshi values except fCurrentSupply is a double, non satoshid.
        if (
            (nCurrentHeight % 1200 == 0 && nMoneySupply <= 10000000000) ||
            (nCurrentHeight % 820 == 0 && nMoneySupply >= 10000000000 && nMoneySupply <= 15000000000) ||
            (nCurrentHeight % 650 == 0 && nMoneySupply >= 15000000000 && nMoneySupply <= MAX_MONEY)
        ) {
            if (fDebug)
                LogPrintf("GetMinStakeAge(): Instant Maturation! nMoneySupply=%.8f minStakeAge=0\n", nMoneySupply);
        ...
        if (
            (nCurrentHeight % 1200 == 0 && nMoneySupply <= TEN_BILLION) ||
            (nCurrentHeight % 820 == 0 && nMoneySupply > TEN_BILLION && nMoneySupply <= FIFTEEN_BILLION) ||
            (nCurrentHeight % 650 == 0 && nMoneySupply > FIFTEEN_BILLION && nMoneySupply <= TWENTY_BILLION)
        ) {
            if (fDebug)
                LogPrintf("GetMinStakeAge(): Instant Maturation! nMoneySupply=%.8f minStakeAge=0\n", nMoneySupply);
    ...
    } else if (nCurrentHeight >= Params().ThreeOhFix() && nCurrentHeight < Params().ThreeTwoFix() ) {
        nMultiplier = fCurrentSupply / ONE_BILLION;
    } else if (nCurrentHeight >= Params().ThreeTwoFix()) {
        // after the ThreeTwoFix stake every 2 hours
        nHours = 2;
        nMultiplier = 1;
    }

    // while on testnet, we would like to keep it in minutes instead of hours
    // for easier testing.
    if (TestNet()) {
        if (fDebug)
            LogPrintf("GetMinStakeAge(): nMoneySupply=%.8f minStakeAge=%dm TESTNET \n", nMoneySupply, (nHours * nMultiplier) * 60);

        return (nHours * nMultiplier) * 60; // return minutes
    }

    if (fDebug)
        LogPrintf("GetMinStakeAge(): nMoneySupply=%.8f minStakeAge=%ds MAINNET \n", nMoneySupply, (nHours * nMultiplier) * 60 * 60);

    return (nHours * nMultiplier) * 60 * 60; // return seconds
}
